...
object GeneratePlots {
  def main(args: Array[String]): Unit = {
    ...
    dumpDurationFrequencies(
      filename = "duration-frequencies.300.tsv",
      durationsFilter = _.filter(_ <= 300)
    )

    dumpMovieDurationFrequencies(
      filename = "duration-frequencies.movies.tsv",
      durationsFilter = _.filter(_ <= 240)
    )

    dumpShortDurationFrequencies(
      filename = "duration-frequencies.shorts.tsv",
      durationsFilter = _.filter(_ <= 60)
    )

    dumpRatingVoteCountFrequencies(
      filename = "rating-vote-count-frequencies.tsv",
    ...
    dumpMovieRatingVoteCountFrequencies(
      filename = "rating-vote-count-frequencies.movies.tsv",
      countsFilter = counts => {
    ...
    for (voteCount <- Seq(5, 10, 100, 1000, 10000)) {
      dumpMinimumRatingsForVoteCount(
        filename = s"minimum-ratings.${voteCount}votes.tsv",
    ...
    for (titleCount <- Seq(250, 1500)) {
      dumpMinimumRatingsForTitleCount(
        filename = s"minimum-ratings.${titleCount}movies.tsv",
        titleType = ImdbStats.MovieType,
  ...
  def dumpDurationFrequencies(
      filename: String,
      durationsFilter: Seq[Double] => Seq[Double]
  ): Unit =
    dumpFrequencies(
      filename,
  ...
  def dumpMovieDurationFrequencies(
      filename: String,
      durationsFilter: Seq[Double] => Seq[Double]
  ): Unit =
    dumpFrequencies(
  ...
  def dumpShortDurationFrequencies(
      filename: String,
      durationsFilter: Seq[Double] => Seq[Double]
  ): Unit =
    dumpFrequencies(
  ...
  def dumpRatingVoteCountFrequencies(
      filename: String,
      countsFilter: Seq[Int] => Seq[Double]
  ): Unit =
    dumpFrequencies(
      filename,
  ...
  def dumpMovieRatingVoteCountFrequencies(
      filename: String,
      countsFilter: Seq[Int] => Seq[Double]
  ): Unit =
    dumpFrequencies(
  ...
  def dumpMinimumRatingsForTitleCount(
      filename: String,
      titleType: String,
      titleCount: Int
  ): Unit = {
